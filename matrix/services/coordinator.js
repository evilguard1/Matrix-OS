...
export function evaluateObjective(data) {
    ...
    if (worldDaemonRooted && hackingLevel >= worldDaemonReqLevel) {
        return {
            id: "W0R1D_D43M0N", title: "Destroy World Daemon",
            ...
            nextStep: "Destroy World Daemon to finish BitNode", etaStr: "READY",
        };
    }

    if (queuedAugs >= 10 || (queuedAugs >= 5 && targetAugPrice > 0 && cash < targetAugPrice)) {
        return {
            id: "INSTALL_AUGMENTATIONS", title: "Install Augmentations",
            reason: `${queuedAugs} augmentations queued for installation`, liquidateStocks: true,
            ...
            nextStep: `Install ${queuedAugs} queued augmentations and restart`, etaStr: "READY",
        };
    }
    ...
    if (factions.includes("Daedalus") && !hasRedPill) {
        if (redPillRep >= redPillReqRep) {
            ...
            return {
                id: "THE_RED_PILL", title: "Acquire The Red Pill",
                ...
                nextStep: `Accumulating ${formatMoney(redPillCost)} for The Red Pill`, etaStr: calcEta(redPillCost),
            };
        }
    }
    ...
    if (!factions.includes("Daedalus") && !hasRedPill && (hackingLevel >= 1500 || totalAssets >= 50_000_000_000)) {
        ...
        return {
            id: "RESERVE_MILESTONE", title: "Daedalus Requirement ($100B)",
            reason: "Accumulating capital for Daedalus faction invitation", liquidateStocks: liquidate,
            ...
            nextStep: `Accumulating ${formatMoney(DAEDALUS_CASH)} for Daedalus invite`, etaStr: calcEta(DAEDALUS_CASH),
        };
    }
    ...
    if (targetAugPrice > 0) {
        ...
        return {
            ...
            nextStep: `Funding ${targetAugName || "augmentation"} (${formatMoney(targetAugPrice)})`, etaStr: calcEta(targetAugPrice),
        };
    }

    if (sf2 && !hasGang && karma > -54) {
        return {
            id: "GANG_KARMA", title: "Karma Rush for Gang",
            reason: `Karma is ${karma.toFixed(1)} / -54.0 required to unlock Gang`, liquidateStocks: false,
            ...
            nextStep: `Accumulating Karma (${karma.toFixed(1)} / -54.0 for Gang)`, etaStr: "IN PROGRESS",
        };
    }

    if (!hasTor && cash >= TOR_COST * 0.5) {
        return {
            id: "BUY_TOR", title: "Purchase TOR Router",
            reason: "Buying TOR Router to unlock Darkweb port-opening programs", liquidateStocks: false,
            ...
            nextStep: `Reaching ${formatMoney(TOR_COST)} for TOR Router`, etaStr: calcEta(TOR_COST),
        };
    }
    if (missingPrograms.length > 0 && programCosts > 0) {
        return {
            id: "BUY_PROGRAMS", title: `Buy Port Programs (${missingPrograms.length} left)`,
            reason: `Acquiring Darkweb programs: ${missingPrograms.join(", ")}`, liquidateStocks: false,
            ...
            nextStep: `Buying ${missingPrograms[0]} (${formatMoney(programCosts)})`, etaStr: calcEta(programCosts),
        };
    }
    ...
    if (!sf3 && !hasCorp && totalAssets >= 50_000_000_000 && totalAssets < CORP_CASH * 1.2) {
        ...
        return {
            id: "RESERVE_MILESTONE", title: "Corporation Bootstrap ($150B)",
            ...
            nextStep: `Accumulating ${formatMoney(CORP_CASH)} for Corporation bootstrap`, etaStr: calcEta(CORP_CASH),
        };
    }

    if (Number.isFinite(homeRamUpgradeCost) && homeRamUpgradeCost > 0 && cash >= homeRamUpgradeCost * 0.5) {
        return {
            id: "EXPAND_RAM", title: `Upgrade Home RAM (${homeRam * 2}GB)`,
            reason: `Expanding Home RAM from ${homeRam}GB to ${homeRam * 2}GB`, liquidateStocks: false,
            ...
            nextStep: `Reaching ${formatMoney(homeRamUpgradeCost)} for ${homeRam * 2}GB Home RAM`, etaStr: calcEta(homeRamUpgradeCost),
        };
    }
    ...
    return {
        id: "BOOTSTRAP_INCOME", title: "Network Expansion & Hacking Income",
        reason: `Building cash reserve and hacking skill (lvl ${hackingLevel})`, liquidateStocks: false,
        ...
        nextStep: `Reaching ${formatMoney(reqCash)} for ${reqLabel}`, etaStr: calcEta(reqCash),
    };
}
...
export function planDirectives(data) {
    ...
    const {
        karma = 0, hasGang = false, resetInfo = {}, hasTor = false,
        homeRam = 8, hackingLevel = 1, targetAugFaction = "",
    } = data;
    ...
    const directives = {
        ...
        singularity:
            ...
            : "rep",
        bladeburner: phase === "ENDGAME" ? "blackops" : "rank",
        stock: liquidate ? "liquidate" : lean ? "hold" : "trade",
    };
...
export async function main(ns) {
    ...
    while (true) {
        ...
        try {
            ...
            await writeJson(ns, COORDINATOR_STATE, { service: "coordinator", updated: Date.now(), ...result });
            await writeJson(ns, DIRECTIVES_STATE, {
                service: "coordinator", updated: Date.now(), phase: plan.phase,
                objectiveId: plan.objectiveId, directives: plan.directives, budgets: plan.budgets,
                liquidateStocks: result.liquidateStocks,
            });
            await writeState(ns, "coordinator", {
                status: "online", objective: result.id, title: result.title, reason: result.reason,
                liquidateStocks: result.liquidateStocks, milestone: result.milestone,
                nextStep: result.nextStep, etaStr: result.etaStr,
