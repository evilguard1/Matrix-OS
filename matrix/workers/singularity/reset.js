...
export async function main(ns) {
 await task(ns,"reset",async()=>{
    const cfg=config(ns);
    if(cfg.progression?.autoInstallAugmentations!==false && !objectivePending(ns) && !terminalLease(ns) && shouldReset(ns,cfg)){
        await event(ns,"singularity",`Installing ${queued(ns)} augmentation(s)`,"success");
        if(!active(ns) || objectivePending(ns) || terminalLease(ns))return;
