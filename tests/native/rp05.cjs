...
(async()=>{
 ...
 try{
  ...
  const result={status:'passed',scope:'Official isolated Bitburner 3.0.1 engine. Synthetic BN4, CyberSec membership, 64GB and $1000. Real supervisor, hacking, Ghost and all eight Singularity tasks over two cycles. No Steam save or actual BN1-to-BN4 transition. RAM peak is sampled, not continuous.',gameSha:'3162fd2590e221eadd0c0fbd46151913f7c4c41c',peakObservedRam:peak,pollingMs:250,completedCycles:cycles.size,dispatcherRam:costs.find(x=>x.file==='matrix/services/singularity.js').ram,workers,state,dispatch,errors,hashes};
