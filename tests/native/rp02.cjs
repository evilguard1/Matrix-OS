...
(async()=>{
 ...
 try {
  ...
  for(const homeRam of [8,16,64,128,256]) {
   ...
   await page.evaluate(s=>globalThis.__ghostHarness.run(`run /rp-install.js --no-start --channel fix/rp-runtime-recovery --release ${s}`),sha);
   ...
   assert.equal(profile.channel,'fix/rp-runtime-recovery');
   ...
   assert.equal(profile.installedSha,sha);assert.equal(profile.channel,'fix/rp-runtime-recovery');assert.equal(journal.phase,'installed');
