...
12. Run `node tests/native/operator.cjs` for the HTTP operator faction adapter. The real gateway writes through an isolated native file adapter, the real agent dispatches the objective, and real faction work crosses the observed augmentation requirement. The fixture seeds reputation one point below that threshold after proving work started. It verifies scoped completion and a single agent execution after replay. Synthetic BN4/64GB/CyberSec/XP/root; no Steam save, production WebSocket or real GPT. Output: `docs/rp/evidence/operator/native.json`.
