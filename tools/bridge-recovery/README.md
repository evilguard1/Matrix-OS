...
Restart the existing Node config process; do not create a second listener. In Bitburner connect localhost:12525 with a five-second reconnect delay. Publish the agent through the native Remote API, verify RAM, then run /cloud/agent.js once. The full Matrix supervisor maintains it afterwards when its file exists. Automatic agent startup below 64 GB is not included.
