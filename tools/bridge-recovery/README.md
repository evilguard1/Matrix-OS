...
Run the standalone mocked regression suite from the repository root:
    ...
    node tools/bridge-recovery/tests/objectives.mjs

It uses a temporary in-memory test token, never the production token, and binds its HTTP test to an ephemeral local port.

The operator routes expose signed pause/resume, observed faction reputation and automatic policy choices, plus durable receipt tracking. See ../../docs/rp/OPERATOR-BRIDGE-1.3.md and ../../docs/rp/OPERATOR-BRIDGE-1.2.md for the exact protocol and limits. These routes reuse the gateway authentication; they are not a separate permission boundary. The existing administrative file/job routes remain available to the same credential.
